...
void callback_lidar_twk2(const sensor_msgs::LaserScan::ConstPtr &msg)
{
   ...
   /* else
    ...
    if (sine >= 0.0f)
    {
        ...
        if( theta > 15 && f_turn == 1 )
        {
            PWM_sv = STEERING_LEFT;
            printf("left\n");
            f_turn = 0;
        }
        else if (theta > 15 && f_stop == 1 )
        {   //PWM_sv = STEERING_CENTER;
            printf("pass\n");
            PWM_sv = STEERING_CENTER + 10;
            f_stop = 0;
        }
        else if ( f_stop == 0 && f_turn == 0)
...
void callback_person(const darknet_ros_msgs::BoundingBoxes::ConstPtr &msg)
{
    ...
    {
        // printf("for ok\n");
        if (std::strcmp(msg->bounding_boxes[i].Class.c_str(), "start") == 0)
        {
            //     count = 1;
            PWM_dc = 340;
            selectedChannel = ESC_CHANNEL;
        ...
        else if (std::strcmp(msg->bounding_boxes[i].Class.c_str(), "left") == 0)
        {
            //     count = 1;
            f_turn = 1;
            //printf("flag_p change! %d\n",flag_p);
